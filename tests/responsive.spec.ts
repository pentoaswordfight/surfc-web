...
test.describe('nav adapts to viewport', () => {
  ...
  test('desktop: inline links are visible, hamburger toggle is hidden', async ({ page }) => {
    ...
    // SUR-679 — the nav now carries a single "Get Marginborn" CTA (.bf-cta) and
...
test.describe('hamburger menu behaviour', () => {
  ...
  test('clicking toggle opens the panel and exposes the nav destinations', async ({ page }) => {
    ...
    // SUR-679 — destinations are How it works / Blog + the single "Get Marginborn"
    ...
    await expect(menu.locator('a', { hasText: 'Get Marginborn' })).toBeVisible()
