...
| `public/robots.txt`, `public/_headers` (Cloudflare cache headers — `/_astro/*` immutable 1y), OG metadata in `src/layouts/BaseLayout.astro` / `src/layouts/BlogPostLayout.astro`, structured data, `src/pages/rss.xml.js`, sitemap configuration in `astro.config.mjs`. **Exception:** the `Content-Type` rules for the two extensionless `.well-known` files belong to the passkey row below — `auth-bridge-reviewer`, not `seo-reviewer` | **GCE** | Founder sign-off | Founder sign-off after `seo-reviewer` persona pass |
