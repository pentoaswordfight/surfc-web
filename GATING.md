...
| `src/lib/appUrl.ts` (signup deep-link → `${PUBLIC_APP_URL}/signin`; SUR-711 dropped `?intent=signup`; targets `/signin` directly to skip the catch-all redirect) | **GCE** | Founder sign-off; cross-check against the React app's AuthScreen | Founder sign-off after `auth-bridge-reviewer` persona pass |
...
| `src/policies/*.md` (**the legal text itself** — in-repo since SUR-618/619 replaced the Termly embed, so editing here *does* change the published policy). The body is a lockstep duplicate of the same path in surfc (no shared artefact — **derive one copy from the other rather than hand-editing both**). Both repos store **LF** in git, but **this repo has no .gitattributes**, so normalisation here depends on each contributor's core.autocrlf rather than being pinned as it is in surfc; on Windows the working tree is CRLF. The "Last updated" date is set at publish time, not per PR. **Only the glob above is backticked on purpose** — every backticked token in a path cell becomes a matchable glob | **GCE only** | Founder sign-off; legal review if the change is material | Founder sign-off after `legal-copy-reviewer` persona pass; add `analytics-privacy-reviewer` when the change touches the consent or telemetry claims |
| `src/pages/policies/privacy.astro`, `src/pages/policies/terms.astro` (the render wrappers — they import the markdown above and own only the layout/title/meta. Policy body text **is** in the static HTML now, and tests/policies.spec.ts asserts on it directly) | **GCE only** | Founder sign-off; green `tests/policies.spec.ts` | Founder sign-off after `legal-copy-reviewer` persona pass |
| `src/pages/policies/delete-account.astro`, `src/pages/policies/data-request.astro`, `src/pages/policies/data-request/confirm.astro` (SUR-782 — the in-house data-request form that replaced the Termly DSAR form, and the page registered in Google Play Console as the app's **Delete account URL**. Added under §5 item 4 (legal copy). The deleted / kept / retention copy on the delete-account page is a legal claim in lockstep with `src/policies/privacy.md` §8 and with the founder-ratified 12-month request-record retention — change §8 and this page together. **The delete-account slug is part of the store listing**: renaming it breaks the listing until Play Console is repointed. The confirm page is noindex and excluded from the sitemap; it POSTs the token only on a button press because mail scanners fetch every link. The confirm token therefore reaches PostHog's current-URL property only after consent, and is single-use / 24h — accepted. **Only the four paths above are backticked on purpose** — see the policies row) | **GCE only** | Green `tests/dsar.spec.ts` + `tests/policies.spec.ts`; founder sign-off; the Play Console URL repoint is a founder step in the surfc-intranet runbook | Founder sign-off after `legal-copy-reviewer` + `ux-reviewer` persona passes |
| `src/lib/dsar.ts`, `src/scripts/dsarForm.ts` (SUR-782 — the client half of the cross-repo contract with surfc's dsar-request Edge Function: field names, the start + confirm routes, the honeypot field, the certification rules, and which server messages are shown verbatim. Sends the apikey header only — no Bearer, requesters need no account. dsarForm.ts is the only script that puts user input on the wire from this site. **Only the two paths above are backticked on purpose**) | **GCE only** | Green `tests/dsar.spec.ts` (every call intercepted) + founder sign-off; cross-check against the surfc function's deno tests | Founder sign-off after `auth-bridge-reviewer` persona pass |
...
| `astro.config.umbrella.mjs`, `src-umbrella/**`, `public/_redirects` (SUR-1062 — the braird.app umbrella site; a SECOND Cloudflare Pages project builds from this one repo, and the build:umbrella / preview:umbrella scripts drive it). The publicDir is shared with the marketing build **on purpose**: braird.app is the permanent passkey Relying Party (SUR-1050 §1), so pointing both builds at one public directory is what stops the well-known files drifting between them — the row below owns those files themselves. Unpicking that sharing, or changing outDir/srcDir, silently changes what the RP origin serves. A _redirects file ships to BOTH sites for the same reason, so per-host redirects need the Cloudflare zone rules row instead (**only real paths are backticked here on purpose — see the policies row**) | **GCE** | Founder sign-off + green `tests/umbrella.spec.ts` | Founder sign-off after `regression-reviewer` persona pass |
...
| `src/pages/robots.txt.ts` (moved out of public/ by SUR-1062 so it stays scoped to the marketing build; its Sitemap line now derives from `site`), `public/_headers` (Cloudflare cache headers — `/_astro/*` immutable 1y), OG metadata in `src/layouts/BaseLayout.astro` / `src/layouts/BlogPostLayout.astro`, structured data, `src/pages/rss.xml.js`, sitemap configuration in `astro.config.mjs`. **Exception:** the Content-Type rules for the two extensionless well-known files belong to the passkey row below (unbackticked on purpose — backticking would glob them onto this weaker row) — `auth-bridge-reviewer`, not `seo-reviewer` | **GCE** | Founder sign-off | Founder sign-off after `seo-reviewer` persona pass |
| `public/.well-known/webauthn` (the WebAuthn Related Origin Requests allow-list), `public/.well-known/apple-app-site-association`, `public/.well-known/assetlinks.json` (native app links and webcredentials). **One Pages build serves braird.app, surfc.app and marginborn.com from this one `public/` dir**, so the ROR file is the allow-list for **both** relying parties at once, and every entry in it is load-bearing for one of them: `https://app.braird.app` is what lets the SUR-687 bridge assert the legacy **surfc.app** passkey (required until surfc.app sunsets, SUR-683), and `https://app.marginborn.com` is what lets that origin assert **braird.app** passkeys at all (SUR-1085). braird.app serves these for the product's lifetime per SUR-1050 §1, so the landing rebrand never retires them. Deleting a file or dropping an origin locks those users out of their E2EE vault, reported only as "PRF unavailable" — unbounded and silent. Two further constraints: browsers process about **five registrable-domain labels** and ignore the rest silently; and the `Content-Type: application/json` rules in `public/_headers` are half this contract, because both ROR and the AASA are extensionless | **GCE only** | Founder sign-off + a real passkey ceremony on every listed origin — that ceremony is the gate for **adding** an origin, because `tests/well-known.spec.ts` only guards removal (its origin list is a local copy, so an origin nobody adds is an origin it cannot miss) | Founder sign-off after `auth-bridge-reviewer` persona pass |
...
|---|---|---|
| `src/components/*.astro` (non-pricing) — `Nav.astro`, `Footer.astro`, `Testimonials.astro`, `CommonplaceCarousel.astro`, `CapturePhoneMockup.astro`, `CapturePassages.astro`, `ClosingCta.astro`, `Faq.astro`, `PulledQuote.astro`, `SupportingCards.astro`, `TableOfContents.astro`, `TagIdeas.astro`, `AuthorCard.astro`, and `src/components/braird/**` (`Hero`, `Beats`, `Close`, `Wordmark` — the front-door set; **the lockup is `Wordmark.astro`, and a change to it is a brand surface: add `naming-reviewer`**) | **CE** | Multi-persona review report (`regression-reviewer` + `ux-reviewer` + `naming-reviewer` if a new public name) |
...
10. Does this change touch a public/.well-known/ file — the WebAuthn ROR
    allow-list, the Apple app-site-association, or assetlinks — or the
    Content-Type rules serving them? → **Spine, GCE only.**
11. None of the above → **Surface, CE.**
...
- `legal-copy-reviewer` — gates `src/policies/*.md` (the policy text
  itself) and `src/pages/policies/*` (the render wrappers)
  — including the SUR-782 `delete-account` and `data-request` pages, whose
  deleted/kept/retention copy is a legal claim
...
| **SUR-365** | Waitlist form removed; `/waitlist` kept as `noindex` sunset page (sitemap-excluded by exact-match regex in `astro.config.mjs`); signup CTAs deep-link to `/signin` (the `?intent=signup` param was dropped by SUR-711) | `seo-reviewer`, `regression-reviewer`, cross-repo |
