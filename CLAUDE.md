...
| Passkey origin trust (SUR-1085) | `public/.well-known/webauthn` — the Related Origin Requests allow-list. This one build serves braird.app **and** surfc.app, so it is the allow-list for both relying parties | `surfc/src/crypto/passkeyEnrollment.js` `defaultRpId()` picks the RP ID from the live hostname; `useKeyManagement.js` `migrateToBrairdRp()` asserts the legacy `surfc.app` RP across domains | **Must be edited in lockstep.** Any new app origin on a different registrable domain needs an entry here *before* users are sent to it. Every current entry is load-bearing — see `GATING.md` §3.1 before removing one. `tests/well-known.spec.ts` catches a removal; nothing catches an omission except a real ceremony. |
